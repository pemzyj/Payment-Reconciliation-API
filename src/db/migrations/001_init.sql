-- 001_init.sql
-- Core schema for the payment reconciliation engine.
-- Multi-tenant-lite: every row that matters is scoped by merchant_id,
-- but there's no separate auth/roles system — just logical separation.
...
-- ---------------------------------------------------------------------------
-- merchants — tenant boundary. Everything else hangs off merchant_id.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- customers — the merchant's own customer, referenced by invoices and used
-- as the fuzzy-match target against incoming sender_name.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- transactions — incoming bank transfers to reconcile against invoices.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- match_attempts — audit log of every candidate score considered for a
-- transaction, so the UI/demo can show *why* something matched (or didn't).
-- ---------------------------------------------------------------------------
...
-- keep invoices.updated_at honest
