...
async function seed() {
    ...
    try {
        ...
        await client.query("TRUNCATE match_attempts, transactions, invoices, customers, merchants RESTART IDENTITY CASCADE");
        const { rows: merchantRows } = await client.query(`INSERT INTO merchants (name) VALUES ($1) RETURNING id`, ["Adire & Co Wholesale"]);
        const merchantId = merchantRows[0].id;
        const customerIds = {};
        for (const c of CUSTOMERS) {
            const { rows } = await client.query(`INSERT INTO customers (merchant_id, name) VALUES ($1, $2) RETURNING id`, [merchantId, c.name]);
        ...
        for (const inv of invoices) {
            const { rows } = await client.query(`INSERT INTO invoices (merchant_id, customer_id, amount, reference_code, due_date)
         VALUES ($1, $2, $3, $4, $5) RETURNING id`, [merchantId, customerIds[inv.customer], inv.amount, inv.reference_code, inv.due_date]);
        ...
        for (const t of transactions) {
            await client.query(`INSERT INTO transactions (merchant_id, amount, sender_name, narration, occurred_at)
         VALUES ($1, $2, $3, $4, $5)`, [merchantId, t.amount, t.sender_name, t.narration, t.occurred_at]);
        }
